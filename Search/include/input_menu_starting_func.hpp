...
int input_handler(int argc, char *argv[],int* k, int* L,int* probes,double* delta,int* M,char (&metricfr)[256],char (&input_file)[256], char (&query_file)[256], char (&output_file)[256],char (&algorithm)[256]);//synarthsh pou elegxei an prepei na diavastei kati mesa apo thn efarmogh
...
int repeat_handler(vec* nvectors, vec* qvectors,char* input_file,char*query_file,char* output_file,Lhashtables *lht,int alg_flag,hypercube* cube);//synarthsh pou einai ypefthinh gia to menu pou emfanizetai sto telos tou kathe treksimatos
#endif